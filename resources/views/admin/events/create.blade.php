...
@section('content')
    <div class="flex flex-row justify-between my-4">
        <p class="text-xl font-semibold">Add New Event</p>
        <a class="button" href="{{ route('events.index') }}">Back</a>
    </div>
    <form action="{{ route('events.store') }}" method="POST" enctype="multipart/form-data" id="submitForm" class="form">
        @csrf
        <div class="mb-6">
            <label for="event_title" class="form-label">Event Title</label>
            <input type="text" id="event_title" name="event_title" class="form-input" value="{{ old('event_title') }}">
            @error('event_title')
                <script>
        ...
        <div class="mb-6">
            ...
            <textarea id="description" rows="2" name="description" class="form-input">{{ old('description') }}</textarea>
            @error('description')
                <script>
